...
export const makeSocket = (config: SocketConfig) => {
	...
	let epoch = 1
	let keepAliveReq: NodeJS.Timeout | undefined
	...
	const end = (error: Error | undefined) => {
		 ...
		 * registered by messages-recv.ts hold closures over the entire
		 * socket scope. Leaving them attached prevents garbage collection
		 * of the old connection's state.
		 */
	...
	const startKeepAliveRequest = () => {
		const scheduleNextPing = () => {
			keepAliveReq = setTimeout(async () => {
				 ...
				 * This catches cases where pings never even get sent.
				 */
				...
				if (ws.isOpen) {
					try {
						/*
						 * CONNECTION STABILITY: Use a short, dedicated ping timeout (10s)
						 * rather than the default 60s query timeout. waitForMessage()
						 * swallows timeouts and returns undefined — so we must explicitly
						 * treat an undefined result as a failure to detect zombie sockets
						 * where the TCP layer is open but the server is not responding
						 * at the application layer.
						 */
						const PING_TIMEOUT_MS = 10_000
						const pongResult = await query(
							{
								tag: 'iq',
								attrs: {
									id: generateMessageTag(),
						...
						if (pongResult === undefined) {
							// Ping timed out silently — server is not responding
							throw new Boom('Ping timed out', { statusCode: DisconnectReason.connectionLost })
						}
